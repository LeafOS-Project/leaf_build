...
if [ "$JENKINS_RELEASETYPE" = "alpha" ] || [ "$JENKINS_RELEASETYPE" = "beta" ]; then
	...
	export LEAF_EXTRAVERSION="$JENKINS_RELEASETYPE"
fi
...
function telegram() {
	if [ "$JENKINS_TELEGRAM" = true ]; then
		RESULT=$(curl -s "https://api.telegram.org/bot$TELEGRAM_TOKEN/$1" -d "chat_id=@leafos_ci" -d "message_id=$MESSAGE_ID" \
			-d "parse_mode=Markdown" -d "sticker=$2" -d "text=$2")
		MESSAGE_ID_TMP=$(jq '.result.message_id' <<<"$RESULT")
		[[ $MESSAGE_ID_TMP =~ ^[0-9]+$ ]] && MESSAGE_ID=$MESSAGE_ID_TMP
	fi
}

function sync() {
	telegram sendMessage "Build started for *$JENKINS_DEVICE*[Jenkins](${BUILD_URL}console)
...
function target-files() {
	telegram editMessage "Build started for *$JENKINS_DEVICE*[Jenkins](${BUILD_URL}console)
Build status: Generating target files"
...
function sign() {
	telegram editMessage "Build started for *$JENKINS_DEVICE*[Jenkins](${BUILD_URL}console)
Build status: Singing build"
	mkdir -p "/var/lib/jenkins/leaf/target-files/$RELEASE_DIR$JENKINS_DEVICE"
	for JENKINS_FLAVOR in "${LEAF_FLAVORS[@]}"; do
		"$OTATOOLS/sign_target_files_apks" -o -d "$KEY_DIR" --avb_vbmeta_key "$KEY_DIR/avb.pem" --avb_vbmeta_algorithm "$AVB_ALGORITHM" \
...
function ota-package() {
	telegram editMessage "Build started for *$JENKINS_DEVICE*[Jenkins](${BUILD_URL}console)
Build status: Generating OTA package"
	for JENKINS_FLAVOR in "${LEAF_FLAVORS[@]}"; do
...
Build status: Uploading"
	...
	for JENKINS_FLAVOR in "${LEAF_FLAVORS[@]}"; do
		retry_uploading "$JENKINS_DEVICE-ota-$JENKINS_FLAVOR-$BUILD_ID.zip"
		if [ -f "$JENKINS_DEVICE-incremental-ota-$JENKINS_FLAVOR-$BUILD_ID.zip" ]; then
			retry_uploading "$JENKINS_DEVICE-incremental-ota-$JENKINS_FLAVOR-$BUILD_ID.zip"
...
function cleanup() {
	if [ "$?" = 0 ]; then
		telegram sendMessage "Build completed successfully"
	else
		telegram sendMessage "Build failed"
	fi
