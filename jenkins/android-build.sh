...
if [ "$JENKINS_RELEASETYPE" = "Alpha" ]; then
	RELEASE_DIR="alpha/"
	export LEAF_EXTRAVERSION="alpha"
elif [ "$JENKINS_RELEASETYPE" = "Beta" ]; then
	RELEASE_DIR="beta/"
	export LEAF_EXTRAVERSION="beta"
fi
...
function upload() {
	up() { while true; do rsync -avP "$1:jenkins@$MASTER_IP:$DL_DIR" && break; sleep 30; done ;}
	ssh jenkins@$MASTER_IP mkdir -p "$DL_DIR"
	for JENKINS_FLAVOR in "${LEAF_FLAVORS[@]}"; do
		up "$JENKINS_DEVICE-ota-$JENKINS_FLAVOR-$BUILD_ID.zip"
		if [ -f "$JENKINS_DEVICE-incremental-ota-$JENKINS_FLAVOR-$BUILD_ID.zip" ]; then
			up "$JENKINS_DEVICE-incremental-ota-$JENKINS_FLAVOR-$BUILD_ID.zip"
			rm -f "$TARGET_FILES_DIR/$(cat "$TARGET_FILES_DIR/latest_$JENKINS_FLAVOR")"
		fi
		up -p "$TARGET_FILES_DIR/$JENKINS_DEVICE-target_files-$JENKINS_FLAVOR-$BUILD_ID-signed.zip" IMAGES/recovery.img >"$JENKINS_DEVICE-recovery-$JENKINS_FLAVOR-$BUILD_ID.img"
		retry "$JENKINS_DEVICE-recovery-$JENKINS_FLAVOR-$BUILD_ID.img"
