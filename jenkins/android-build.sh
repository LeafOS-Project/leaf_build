...
case "$1" in
	...
	target-files-package)
		...
		for JENKINS_FLAVOR in $LEAF_FLAVORS; do
			unset WITH_GMS
			unset WITH_MICROG
			[ $JENKINS_FLAVOR = "GMS" ] && export WITH_GMS=true
			[ $JENKINS_FLAVOR = "microG" ] && export WITH_MICROG=true
			rm -rf $OUT
	...
	sign)
		...
		for JENKINS_FLAVOR in $LEAF_FLAVORS; do
			./out/host/linux-x86/bin/sign_target_files_apks -o -d "$KEY_DIR" --avb_vbmeta_key "$KEY_DIR/avb.pem" --avb_vbmeta_algorithm $AVB_ALGORITHM \
				...
				out/$JENKINS_DEVICE-target_files-$JENKINS_FLAVOR-$BUILD_ID \
	...
	ota-package)
		...
		;;
	upload)
		ssh jenkins@$MASTERIP mkdir -p $DL_DIR
		for JENKINS_FLAVOR in $LEAF_FLAVORS; do
			scp $JENKINS_DEVICE-ota-$JENKINS_FLAVOR-$BUILD_ID.zip jenkins@$MASTERIP:$DL_DIR
			if [ -f $JENKINS_DEVICE-incremental-ota-$JENKINS_FLAVOR-$BUILD_ID.zip ]; then
				scp $JENKINS_DEVICE-incremental-ota-$JENKINS_FLAVOR-$BUILD_ID.zip jenkins@$MASTERIP:$DL_DIR
			...
			scp $JENKINS_DEVICE-recovery-$JENKINS_FLAVOR-$BUILD_ID.img jenkins@$MASTERIP:$DL_DIR
