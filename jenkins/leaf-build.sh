...
if [ "$JENKINS_RELEASETYPE" != "release" ]; then
...
Build type: $JENKINS_RELEASETYPE
Build status:"
...
function sync() {
	...
	repo init -u https://git.leafos.org/LeafOS-Project/android -b "leaf-$LEAF_VERSION" --depth=1
...
function target-files() {
	...
	if [ "$JENKINS_LUNCH" ]; then
		lunch "${JENKINS_LUNCH}_$JENKINS_DEVICE-${JENKINS_BUILDTYPE:-user}"
	else
		lunch "$JENKINS_DEVICE-${JENKINS_BUILDTYPE:-user}"
	fi
...
function ota-package-name() {
	LEAF_PACKAGE="leaf-$LEAF_VERSION$LEAF_EXTRAVERSION-$BUILD_ID-$JENKINS_FLAVOR-$JENKINS_DEVICE"
	LEAF_INCR_PACKAGE="leaf-$LEAF_VERSION$LEAF_EXTRAVERSION-$OLD_BUILD_ID-incr-$BUILD_ID-$JENKINS_FLAVOR-$JENKINS_DEVICE"
}
...
function cleanup() {
	if [ "$BUILD_STATUS" = "SUCCESS" ]; then
		telegram editMessageText "$TELEGRAM_MESSAGE Completed
[Download](https://${DL_DIR/\/var\/www\//})"
	else
		telegram editMessageText "$TELEGRAM_MESSAGE Failed at $STAGE_NAME"
	fi
