...
[[ "$JENKINS_DEVICE" = *gsi* ]] && TARGET_IS_GSI="true"
...
function ota-package() {
	...
	for JENKINS_FLAVOR in "${LEAF_FLAVORS[@]}"; do
		...
		if [ -z "$TARGET_IS_GSI" ]; then
			...
			"$OTATOOLS/img_from_target_files" -k "$KEY_DIR/releasekey" \
				"$TARGET_FILES_DIR/$JENKINS_DEVICE-target-files-$JENKINS_FLAVOR-$BUILD_ID-signed.zip" \
				"$LEAF_PACKAGE-img.zip"

			# Cleanup since there are no incremental packages for GSI
			rm "$TARGET_FILES_DIR/$JENKINS_DEVICE-target-files-$JENKINS_FLAVOR-$BUILD_ID-signed.zip"
			rmdir "$TARGET_FILES_DIR" 2>/dev/null
		fi
	done
}
