...
TELEGRAM_MSG_TEMPLATE="Build #$BUILD_NUMBER started for *$JENKINS_DEVICE*: [Jenkins](${BUILD_URL}console)
Build status:"
...
function sync() {
	telegram sendMessage "$TELEGRAM_MSG_TEMPLATE Syncing"
...
function target-files() {
	telegram editMessageText "$TELEGRAM_MSG_TEMPLATE Generating target files"
...
function sign() {
	telegram editMessageText "$TELEGRAM_MSG_TEMPLATE Signing build"
...
function ota-package() {
	telegram editMessageText "$TELEGRAM_MSG_TEMPLATE Generating OTA package"
	for JENKINS_FLAVOR in "${LEAF_FLAVORS[@]}"; do
...
function upload() {
	telegram editMessageText "$TELEGRAM_MSG_TEMPLATE Uploading"
	retry_uploading() {
		for ((n = 0; n < 3; n++)); do
